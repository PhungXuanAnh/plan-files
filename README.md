...
Bounded `overview`/`resume-pack` output is capped at 4 KiB and supplies targeted follow-up reads for truncated sections. `restore-check` validates semantic resume fields plus handoff/external-evidence freshness before operational work. PostTool reminders use semantic risk classes, so read-only exploration and plan maintenance do not create false stale pressure while likely evidence and mutations still require timely checkpoints. Early validation also catches malformed legacy plans, missing profiles and inconsistent phase status before operational tools. PostTool repeats unresolved repair warnings, while healthy progress/finalization advisories share the semantic/time debounce; unchanged reads do not force an injection. Short planning commands run in the foreground; background builds/tests remain supported. Hook messages name the absolute path of every script and document they ask for, so the agent never guesses an install location or searches for a skill script, and the ownership message offers every routing verb its gate accepts, including `discuss` for a question that implements nothing. Operational work on an owned plan also requires the skill to have been read once in the session, and a plan whose phases are all settled (complete, blocked, or deferred) must be reopened for newly authorized work — `plan_edit.py reopen` records the authorization in `decisions.md`, opens the phase that carries the work, and starts its first item in one transaction — before it accepts further mutation, so a new prompt cannot implement and hand off against a plan that still describes finished work. See the [early enforcement matrix](skills/plan-files/references/routing-and-hooks.md#early-enforcement-and-stop-audit).
...
5. Start with one useful phase. Read `plan_state.py overview` and follow only needed targets; `restore.ok: true` needs no immediate duplicate restore-check. Edit short prose directly. Use fingerprinted commands for structure: `phase-add --item ... --verify ... --start` assembles and starts the phase in one write. Global `--compact` returns short editor JSON; `fingerprint --file` returns the full edit hash, and `--json` returns both hashes.
...
When compacting findings, preserve detail in a linked file and leave room for the next update. Native Edit/Write, a scoped literal `cat` heredoc with a quoted delimiter, and `section-replace` on a unique existing findings heading (including a legacy heading) are supported repair paths. Arbitrary inline Python remains opaque to the maintenance gate; its denial now names usable alternatives. See [plan operations](skills/plan-files/references/plan-operations.md).

PostTool counts unknown calls separately from recognized evidence and mutations. Unknown activity no longer triggers an early stale warning; after three minutes it can request a conditional checkpoint review. Evidence-based early reminders and Stop enforcement remain active.
