#!/bin/bash
# planning-with-files: Agent stop hook for Kiro
# Checks if all phases in task_plan.md are complete.
# Injects continuation context if phases are incomplete.
# Always exits 0 — outputs to stdout for "agent" type hooks.
# Debug log written to tmp/hook-logs/plan-with-files/agent-stop.log
#
...
# Skip silently (emit nothing and exit 0) if ANY of the following is true:
...
# Per Kiro docs: if we previously output a message and the agent is stopping
# again (after processing our message), a stop_hook_active flag may be present.
# Check for this to avoid infinite loops.
...
if [ "$STOP_HOOK_ACTIVE" = "true" ]; then
    log "decision: GUARDED (stop_hook_active=true) -> silent exit to break loop"
    exit 0
fi

if [ ! -f "$PLAN_FILE" ]; then
    log "${PLAN_FILE:-task_plan.md}: ABSENT -> silent exit (no-op)"
    exit 0
fi
...
if [ $((COMPLETE + DEFERRED)) -ge "$TOTAL" ]; then
    ...
    log "decision: ALL SETTLED (complete=$COMPLETE deferred=$DEFERRED total=$TOTAL) -> allow stop"
    exit 0
fi
...
if [ "$COMPLETE" -eq 0 ] && [ "$IN_PROGRESS" -eq 0 ] && [ "$DEFERRED" -eq 0 ]; then
    log "decision: PLANNING MODE (no phases started/complete/deferred, all pending) -> allow stop"
    exit 0
fi
...
if [ -z "${PHASE_NUM:-}" ] && [ "$COMPLETE" -eq 0 ] && [ "$DEFERRED" -eq 0 ]; then
    log "decision: DISCUSSION MODE (Current Phase empty, nothing complete/deferred) -> allow stop"
    exit 0
fi

# Task incomplete -> output continuation message
