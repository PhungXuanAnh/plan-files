#!/bin/bash
# planning-with-files: Post-tool-use hook for Kiro
...
# Always exits 0 - outputs to stdout. Debug log written to
...
if [ ! -f "$PLAN_FILE" ]; then
    log "${PLAN_FILE:-task_plan.md}: ABSENT -> emitting nothing (no-op, zero pollution)"
    exit 0
fi
...
if [ "$EMIT_NUDGE" = "false" ]; then
    log "debounce active -> silent exit"
    exit 0
fi
...
log "context output: ${#CONTEXT} chars"
log "--- context begin ---"
{
    flock -x 9 || true
    printf '%s\n' "$CONTEXT" >> "$LOG_FILE"
...
# Output context directly to stdout (Kiro will inject it automatically)
printf '%s\n' "$CONTEXT"
exit 0
