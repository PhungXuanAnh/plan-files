...
Store private planning state under `<project-root>/tmp/plan-with-files/<task-id>/`:
...
Create new files from [the templates](templates/). Keep `tmp/` and the root `.plan-with-files` pointer out of version control. Task ids use only letters, digits, `-`, `_`, or `.`.
...
5. For a new task, create the three required files. Hooks auto-claim after `tasks.md` exists; without ownership hooks, update `.plan-with-files` manually.
...
4. Write durable discoveries to `findings.md` after every two view/browser/search operations. Read `decisions.md` before changing it; preserve superseded choices and open questions.
5. Log errors immediately, diagnose them, and change approach. Try three materially different actionable paths before treating an external dependency as a blocker.
...
Read output supplies SHA-256 fingerprints. Pass the target fingerprint to `plan_edit.py`; stale, invalid, unsafe, or budget-worsening edits fail closed. Use `--dry-run` for consequential structure changes. Direct Markdown reads/edits remain valid for uncommon repair or narrative restructuring. Keep `plan_checkpoint.py` as the execution-transition API.
