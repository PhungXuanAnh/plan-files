...
Log errors immediately with attempt and resolution. A failed click, selector, timeout, rejected execution route, or tool path is not an external blocker while a materially different path remains. Diagnose before retrying and attempt three materially different viable approaches before declaring an external impasse.
...
If a true dependency blocks one phase, update Resume Checkpoint, use `blocked (reason)`, and continue other phases. Use `deferred (reason)` only on explicit user instruction.
