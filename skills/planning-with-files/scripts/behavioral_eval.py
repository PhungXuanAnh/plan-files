...
def _write_fixture(project: Path) -> Path:
    task_dir = project / "tmp/plan-with-files/eval-task"
    ...
    (project / ".plan-with-files").write_text("eval-task\n", encoding="utf-8")
...
def _hook_probe(project: Path, scripts: Path) -> dict[str, object]:
    ...
    _run([str(state_tool), "bind", "eval-task"], env=bind_env)
    post_hook = repo / ".codex/hooks/planning-with-files/scripts/post-tool-use.sh"
    stop_hook = repo / ".codex/hooks/planning-with-files/scripts/agent-stop.sh"
    ...
    handoff = project / "tmp/plan-with-files/eval-task/handoff.md"
    handoff.write_text("# Handoff\n\nUpdated: stale fixture\n", encoding="utf-8")
    os.utime(handoff, (1, 1))
    warning_env = os.environ.copy() | {
        "COMMON": str(repo / ".codex/hooks/planning-with-files/scripts/common.sh"),
