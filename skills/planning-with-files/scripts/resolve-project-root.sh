#!/usr/bin/env bash
# planning-with-files: resolve the true workspace root for hook scripts.
#
...
#      `.plan-with-files` pointer file (the file the skill already keeps at a
#      project root to name the active task — present, even empty, is
#      enough; no separate marker file is needed). FARTHEST (outermost)
#      match wins: a `.plan-with-files` can legitimately exist at more than
...
#      `.plan-with-files` anywhere yet, `touch .plan-with-files` at the
...
outermost=""
while :; do
    [ -e "$dir/.plan-with-files" ] && outermost=$dir
    [ "$dir" = "/" ] && break
    parent=$(dirname "$dir")
    [ "$parent" = "$dir" ] && break
    dir=$parent
