...
Log errors immediately, but only those that changed the approach, then retain them by value rather than age. An unresolved error remains hot; a reusable workaround becomes a finding; a resolved failure with audit value becomes history; operational noise can disappear after its phase completes.
