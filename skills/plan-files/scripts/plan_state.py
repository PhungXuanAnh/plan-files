#!/usr/bin/env python3
"""Parse and validate plan-files outcome-item state."""
...
def _overview_dump(payload: dict[str, object]) -> str:
    # _compact_meta is an internal degradation flag, never part of the schema.
    visible = {key: value for key, value in payload.items() if key != "_compact_meta"}
...
def _overview_metadata(
    ...
    if payload.get("_compact_meta"):
        truncated = {key: {"omitted": bool(value.get("omitted"))} for key, value in truncated.items()}
        targets = {key: [value[0], value[1]] for key, value in targets.items() if key in truncated}
    return {
        "schema_version": 2,
...
def overview_payload(plan: Path, max_chars: int, total_max_chars: int = DEFAULT_OVERVIEW_CHARS) -> dict[str, object]:
    ...
    # Last resort: shed structural detail rather than fail. A resume that
    # returns nothing is strictly worse than one that returns the restore-
    # critical core and names the follow-up read, and a plan large enough to
    # hit this is exactly when the caller most needs orientation.
    degraded: list[str] = []
    if len(rendered) > total_max_chars:
        # Per-section truncation bookkeeping is the first thing to go: on a
        # heavily trimmed plan it grew larger than the content it described,
        # which is what pushed overview over its own cap and made it fail.
        payload["_compact_meta"] = True
        degraded.append("truncation_detail")
        rendered = _render_overview(payload, sections, total_max_chars)
    if len(rendered) > total_max_chars:
        payload["fingerprints"] = {
            name: value
            for name, value in dict(payload["fingerprints"]).items()
            if name == plan.name
        }
        degraded.append("fingerprints")
        rendered = _render_overview(payload, sections, total_max_chars)
    if len(rendered) > total_max_chars:
        payload["phases"] = [
            {key: entry[key] for key in ("phase", "status", "unchecked")}
            for entry in list(payload["phases"])
        ]
        degraded.append("phase_titles")
        rendered = _render_overview(payload, sections, total_max_chars)
    if len(rendered) > total_max_chars:
        active_phase = state.current_phase
        payload["phases"] = [
            entry for entry in list(payload["phases"]) if entry.get("phase") == active_phase
        ]
        degraded.append("non_current_phases")
        rendered = _render_overview(payload, sections, total_max_chars)
    if degraded:
        meta = payload["view_meta"]
        assert isinstance(meta, dict)
        ...
        assert isinstance(next_read, dict)
        next_read["phases"] = "plan_state.py phase PLAN N"
        next_read["fingerprints"] = "plan_state.py fingerprint FILE"
        meta["serialized_chars"] = len(_overview_dump(payload))
    return payload
...
COMMAND_HELP = {
    ...
    "overview": "bounded resume packet: goal, identity, active item, next action, phase counts, restore issues",
    "resume-pack": "overview plus the sections a fresh session needs first",
    "section": "print one '## Heading' body from any planning file",
    "phase": "print one phase block from the plan",
    "item": "print one P/V item with its evidence",
...
def _parser() -> argparse.ArgumentParser:
    ...
    for command in ("overview", "resume-pack"):
        ...
        overview.add_argument(
            ...
            help=(
                f"serialized cap; 0 means unbounded, otherwise at least {MIN_OVERVIEW_CHARS}. "
                "Over-budget state is degraded and reported in view_meta.degraded, never an error."
            ),
        )

    section = subparsers.add_parser("section", help=COMMAND_HELP["section"])
    section.add_argument("file", type=Path, help="planning file, e.g. decisions.md")
    section.add_argument("heading", help="heading text without the leading '## '")
