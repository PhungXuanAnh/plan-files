...
The new user request can supersede an earlier research-only deliverable or implementation non-goal. This is a scope update within SAME when the user explicitly continues that plan, not a reason to release it. After bind, record the authorization in `decisions.md` (`decision-supersede` retires the superseded row with its replacement and reason), reconcile Goal/Task Identity/Workflow Profile and remaining phases, then implement. When every phase is already complete, PreTool blocks operational mutation until the plan is reopened with the phase that will carry the work; `plan_edit.py reopen` does that in one call — the decision, the scope fields, the phase, and its started first item — and reads and owned-plan repair stay available meanwhile. A plan finalized with `--deactivate-pointer` stops nominating itself instead. Preserve all other non-goals; they are not automatically temporary research constraints.
...
- PreTool gates ambiguous ownership, an unloaded skill, invalid format/profile/item/status/restore state (including legacy plans), explicit background planning helpers, and over-budget unrelated mutations while allowing read-only diagnosis and owned-plan repair. It recognizes both from the tool input, never from a tool or script name: a call passes when it is demonstrably read-only, or when its write targets are inside the owned plan directory. For a tool whose arguments the gate cannot parse, a whole argument must be a path inside that directory; a plan path quoted inside a longer string is a mention, not a target, and grants nothing. A shell command passes only when every one of its segments either runs a planning helper for the owned plan or is itself read-only, so chaining unrelated work onto a repair blocks the whole command rather than smuggling it through. Helpers resolve the owned plan from the pointer, so `--plan` is optional there. Every block message states this condition, so treat it as the instruction rather than as a broken gate.
...
| Every phase complete, with no blocked/deferred work | Block operational mutation until `reopen` records the authorization and opens the phase carrying the new work; allow reads, owned-plan repair, and finalization | Repeat finalization/POINTER_ACTIVE reminder, and the reopen diagnosis after any call with non-zero semantic weight | Existing lease settlement behavior is preserved; assert-finalizable remains required |
