#!/usr/bin/env bash
# Forwards to the canonical resolver in skills/plan-files/scripts/.
...
# the same $HOME/.claude/hooks/plan-files symlink the other hook
# scripts already rely on, without a separate dependency on the skill being
# installed too.
set -u
REPO_ROOT=$(CDPATH= cd -P -- "$(dirname -- "${BASH_SOURCE[0]}")/../../../.." && pwd)
exec bash "$REPO_ROOT/skills/plan-files/scripts/resolve-project-root.sh" "$@"
