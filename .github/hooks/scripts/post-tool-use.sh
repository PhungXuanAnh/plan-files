...
extract_section() {
    # $1 = section name (e.g. 'Goal' or 'Current Phase')
    # Reads $PLAN_FILE, prints body of '## <name>' section (without the
    # header itself), HTML comments stripped, leading/trailing blank lines
    # collapsed.
    awk -v name="$1" '
      $0 ~ "^## " name "[[:space:]]*$" { capture=1; next }
      /^## / { capture=0 }
...
GOAL_BODY=$(extract_section 'Goal')
PHASE_BODY=$(extract_section 'Current Phase')

GOAL_BODY=$(cap "$GOAL_BODY" $MAX_GOAL_CHARS)
PHASE_BODY=$(cap "$PHASE_BODY" $MAX_PHASE_CHARS)
