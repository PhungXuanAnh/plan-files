...
printf '[%s] event=UserPromptSubmit cwd=%s input_bytes=%s\n' "$(date -u +"%Y-%m-%dT%H:%M:%SZ")" "$(pwd)" "${#INPUT}" >> "$LOG_FILE" 2>/dev/null || true
CONTEXT=$(printf '%s' "$INPUT" \
    | bash "$REPO_ROOT/skills/plan-files/scripts/prompt-candidate.sh" codex "$BIND_TOOL")
