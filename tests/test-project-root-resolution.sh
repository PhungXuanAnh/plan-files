...
# --- Tier 2 fallback: real git submodule, no .plan-files anywhere ------
SUB="$TEST_DIR/sub"
mkdir -p "$SUB"
...
#     outer folder has a .plan-files pointer ----------------------------
WS2="$TEST_DIR/ws2"
mkdir -p "$WS2/repoA"
(cd "$WS2/repoA" && git init -q && git commit -q --allow-empty -m init)
printf 'SOME-TASK\n' > "$WS2/.plan-files"
assert_eq "$(bash "$RESOLVER" "$WS2/repoA")" "$WS2" \
    "a plain non-git folder's existing .plan-files must resolve as root even with no git relationship to the inner repo"

# --- Tier 1 ambiguity: BOTH the outer root and an inner child repo have their
#     own .plan-files (for example because a session was once opened
...
#     .plan-files anywhere yet: an empty .plan-files at the intended
...
assert_eq "$(bash "$RESOLVER" "$WS6/repoA")" "$WS6" \
    "an empty .plan-files at the intended root must resolve as root before any task exists"
...
mkdir -p "$WS8/tmp/plan-files/DEMO"
cat > "$WS8/tmp/plan-files/DEMO/tasks.md" <<'EOF'
## Task Identity
demo
## Goal
